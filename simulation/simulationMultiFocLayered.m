...
% This example simulates layered media, with multifocal transmission 
% and focal number 2.
% Created on June 25th, 2024
% Author: Sebastian Merino

function [] = simulationMultiFocLayered(BaseDir)
...
source_focus_points = [10e-3, 20e-3, 30e-3];    % focal length [m]
...
for iSim = 1:length(simuNames)
    ...
    %% SOURCE
    iFocus = 1;
    for source_focus = source_focus_points
        ...
        rf(:,:,iFocus) = bf_data_final(offset:end,:); iFocus = iFocus + 1;
        z = axAxis(offset:end);
        x = latAxis;
